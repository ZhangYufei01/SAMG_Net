...
class BaseReaderWriter(ABC):
    ...
    @abstractmethod
    def read_images(self, image_fnames: Union[List[str], Tuple[str, ...]],semi_fnames: Union[List[str], Tuple[str, ...]]) -> Tuple[np.ndarray, dict]:
        """
...
class SimpleITKIO(BaseReaderWriter):
    ...
    def read_images(self, image_fnames: Union[List[str], Tuple[str, ...]]) -> Tuple[np.ndarray, dict]:
        ...
        for f in image_fnames:
            ...
            # 为了匹配预处理，暂时变一下维度
            #print('here',npy_image.shape)
            #npy_image = np.transpose(npy_image, (0, 2, 3, 1))
            #print('in itk, data.shape:',npy_image.shape)
        ...
        return np.vstack(images).astype(np.float32), dict
