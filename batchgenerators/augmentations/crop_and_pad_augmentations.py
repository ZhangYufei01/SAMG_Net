...
def center_crop(data, crop_size, seg=None,  semi=None):
    #print('center_crop')
    return crop(data, seg, semi, crop_size, 0, 'center')
...
def crop(data, seg=None, semi=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    if semi is not None:
        semi_shape = tuple([len(semi)] + list(semi[0].shape))
        semi_dtype = semi[0].dtype

        if not isinstance(semi, (list, tuple, np.ndarray)):
            raise TypeError("data has to be either a numpy array or a list")

        assert all([i == j for i, j in zip(semi_shape[2:], data_shape[2:])]), "data and seg must have the same spatial " \
                                                                             "dimensions. Data: %s, seg: %s" % \
                                                                             (str(data_shape), str(semi_shape))
    ...
    if seg is not None:
        ...
        seg_return = None
    if semi is not None:
        semi_return = np.zeros([semi_shape[0], semi_shape[1]] + list(crop_size), dtype=semi_dtype)
    else:
        semi_return = None
    ...
    for b in range(data_shape[0]):
        ...
        if semi_return is not None:
            slicer_semi = [slice(0, semi_shape_here[1])] + [slice(lbs[d], ubs[d]) for d in range(dim)]
            semi_cropped = semi[b][tuple(slicer_semi)]
        ...
        if any([i > 0 for j in need_to_pad for i in j]):
            ...
            if semi_return is not None:
                semi_return[b] = np.pad(semi_cropped, need_to_pad, pad_mode_seg, **pad_kwargs_seg)
        else:
            data_return[b] = data_cropped
            if seg_return is not None:
                seg_return[b] = seg_cropped
            if semi_return is not None:
                semi_return[b] = semi_cropped

    return data_return, seg_return, semi_return


def random_crop(data, seg=None, semi=None, crop_size=128, margins=[0, 0, 0]):
    #print('random_crop')
    return crop(data, seg, semi, crop_size, margins, 'random')
