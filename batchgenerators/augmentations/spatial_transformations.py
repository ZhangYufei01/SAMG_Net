...
def augment_mirroring(sample_data, sample_seg=None, sample_semi=None,axes=(0, 1, 2)):
    ...
    if 0 in axes and np.random.uniform() < 0.5:
        ...
        if sample_semi is not None:
            sample_semi[:, :] = sample_semi[:, ::-1]
    ...
    if 2 in axes and len(sample_data.shape) == 4:
        if np.random.uniform() < 0.5:
            ...
            if sample_semi is not None:
                sample_semi[:, :, :, :] = sample_semi[:, :, :, ::-1]
    return sample_data, sample_seg, sample_semi
...
def augment_spatial(data, seg, semi_infor, patch_size, patch_center_dist_from_border=30,
    ...
    if dim == 2:
        ...
        data_result = np.zeros((data.shape[0], data.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                               dtype=np.float32)
    semi_infor_result = None
    if semi_infor is not None:
        if dim == 2:
            semi_infor_result = np.zeros((semi_infor.shape[0], semi_infor.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
        else:
            semi_infor_result = np.zeros((semi_infor.shape[0], semi_infor.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            if seg is not None:
                for channel_id in range(seg.shape[1]):
                    seg_result[sample_id, channel_id] = interpolate_img(seg[sample_id, channel_id], coords, order_seg,
                                                                        ...
                                                                        is_seg=True)
            if semi_infor is not None:
                for channel_id in range(semi_infor.shape[1]):
                    semi_infor_result[sample_id, channel_id] = interpolate_img(semi_infor[sample_id, channel_id], coords, order_seg,
                                                                     border_mode_seg, cval=border_cval_seg)
        else:
            if seg is None:
                s = None
            ...
            if semi_infor is None:
                semi = None
            else:
                semi = semi_infor[sample_id:sample_id + 1]
            if random_crop:

                margin = [patch_center_dist_from_border[d] - patch_size[d] // 2 for d in range(dim)]
                #print(f'random crop')
                d, s, semi = random_crop_aug(data[sample_id:sample_id + 1], seg=s, semi=semi, crop_size=patch_size, margins=margin)
            else:
                #print(f'center crop')
                d, s, semi = center_crop_aug(data[sample_id:sample_id + 1], crop_size=patch_size, seg=s, semi=semi)

            data_result[sample_id] = d[0]
            if seg is not None:
                seg_result[sample_id] = s[0]
            if semi_infor is not None:
                semi_infor_result[sample_id] = semi[0]
    return data_result, seg_result, semi_infor_result
