...
class MirrorTransform(AbstractTransform):
    ...
    def __init__(self, axes=(0, 1, 2), data_key="data", label_key="seg",semi_key="semi_infor",p_per_sample=1):
        self.p_per_sample = p_per_sample
        self.data_key = data_key
        self.label_key = label_key
        self.semi_key = semi_key
        self.axes = axes
        if max(axes) > 2:
    ...
    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        seg = data_dict.get(self.label_key)
        semi = data_dict.get(self.semi_key)
        ...
        for b in range(len(data)):
            if np.random.uniform() < self.p_per_sample:
                ...
                if semi is not None:
                    sample_semi = semi[b]
                ret_val = augment_mirroring(data[b], sample_seg, sample_semi, axes=self.axes)
                data[b] = ret_val[0]
                if seg is not None:
                    seg[b] = ret_val[1]
                if semi is not None:
...
class SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size, patch_center_dist_from_border=30,
                 ...
                 label_key="seg", semi_infor_key='semi_infor',p_el_per_sample=1, p_scale_per_sample=1, p_rot_per_sample=1,
        ...
        self.data_key = data_key
        self.label_key = label_key
        self.semi_infor_key = semi_infor_key
    ...
    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        seg = data_dict.get(self.label_key)
        semi_infor = data_dict.get(self.semi_infor_key)
        ...
        ret_val = augment_spatial(data, seg, semi_infor, patch_size=patch_size,
        ...
        if semi_infor is not None:
            data_dict[self.semi_infor_key] = ret_val[2]
