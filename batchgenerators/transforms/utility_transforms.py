...
class RemoveLabelTransform(AbstractTransform):
    ...
    def __init__(self, remove_label, replace_with=0, input_key="seg", output_key="seg",input_semi='semi_infor', output_semi='semi_infor'):
        self.output_key = output_key
        self.input_key = input_key
        self.replace_with = replace_with
        self.remove_label = remove_label
        self.input_semi = input_semi
        self.output_semi = output_semi

    def __call__(self, **data_dict):
        seg = data_dict[self.input_key]
        semi_infor = data_dict[self.input_semi]
        seg[seg == self.remove_label] = self.replace_with
        semi_infor[semi_infor == self.remove_label] = self.replace_with
        data_dict[self.output_key] = seg
        data_dict[self.output_semi] = semi_infor
        return data_dict
